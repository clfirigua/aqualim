var slideShow=function()
{
	...
	return{
		init:function(){
			...
			ys=this.toppos(ta);ye=ys+ta.offsetHeight;len=t.length;tar=[];
			for(i=0;i<len;i++){
				var id=t[i].value;tar[i]=id;t[i].onclick=new Function("slideShow.getimg('"+id+"')");
											...
											var c=0;for(key in tar){
												...
												if(tar[parseInt(c)+d]){
													...
													ia.timer=setInterval(function(){
														...
														if(i.complete){
															i.av=i.av+fs;i.style.opacity=i.av		 /100;i.style.filter='alpha(opacity='+i.av+')'}if(i.av>=100){if(auto){this.auto()};clearInterval(i.timer);ci=i}},fdout:function(i){i.av=i.av-fs;i.style.opacity=i.av/100;
															...
															if(i.av<=0){
																...
																var taw,taa,len;taw=ta.parentNode.offsetWidth;
																taa=taw/4;bxs=slideShow.leftpos(ta);
																bxe=bxs+taa;fxe=bxs+taw;fxs=fxe-taa;
																},pos:function(e){
																	xp=ie?event.clientX+document.documentElement.scrollLeft:e.pageX;yp=ie?event.clientY+document.documentElement.scrollTop:e.pageY;
